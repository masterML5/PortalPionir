		...
		if(isset($_POST['submit']))
		{
			if(isset($_GET['go']))
			{
				if(preg_match("/[A-Z | a-z | 0-9]+/", $_POST['name']))
				{
					...
					$query = "select * from imenik WHERE ime LIKE '%" . $name . "%' OR prezime LIKE '%" . $name ."%' OR tel_mobilni LIKE '%" . $name ."%'  OR tel_fiksni LIKE '%" . $name ."%'  OR tel_lokal LIKE '%" . $name ."%' OR nazoj LIKE '%" . $name ."%' order by lice_sluzba, prezime, ime, nazoj";
